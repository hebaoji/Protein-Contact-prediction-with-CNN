...
##################################################RESHAPE
	...
	PARTSF2=tf.matmul(PARTSF1drop, weights['SF2']) + biases['SFbiase2']
######################################################################GRAPHE
#	cross_entropy = tf.reduce_mean(-native * tf.log(PARTSF2+0.001))
	cross_entropy=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=PARTSF2, labels=native))
...
##################native SHAPE[10000,2]
			...
			for j in range(10000):
				if(tmp[j,0]==0 and tmp[j,1]==0):
					NATIVEBATCH[j,0]=0.016
					NATIVEBATCH[j,1]=0
					NATIVE1[j,0]=1
					count+=1
				else:
					NATIVEBATCH[j,0]=0
					NATIVEBATCH[j,1]=0.984
					NATIVE1[j,0]=1
			for j in range(10000):
				if(tmp[j,0]==0 and tmp[j,1]==0):		
...
###################################################################)
			if i%50 == 0:
				...
				tmp=sess.run(PARTSF2,feed_dict={contactm:CONBATCH, SS:SSBATCH, SA:SABATCH, colstats:COLBATCH, native:NATIVEBATCH, keep_prob:1.0,referen:NATIVE1})
				tmp1=np.array(tmp)
				print tmp1.sum()/10000.0
				los=sess.run(cross_entropy,feed_dict={contactm:CONBATCH, SS:SSBATCH, SA:SABATCH, colstats:COLBATCH, native:NATIVEBATCH, keep_prob:1.0,referen:NATIVE1})
				print "step %d, training accuracy %g,loss function %g" % (i, train_accuracy,los)
